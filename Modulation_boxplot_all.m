...
class = {'high', 'low'};
set = {'Y_Centroid', 'Y_Flatness','Y_Slope', 'Y_Entropy','Y_Spread','Y_Crest', 'Y_Kurtosis', 'Y_Roll','Y_Skewness'}
...
for s = 1:length(set)
    ...
    title(strcat('Impact of Modulation', string(set(s)), string(i*2),'  Hz on amount of noise on', class{c} ,'_Arousal' ))
    xlabel('clean and SNR levels')
    ylabel(strcat('Modulation ',string(set(s))))
    %title('Modulation specrogram fo')
    specFolder =fullfile('//media/amrgaballah/Backup_Plus/exp_j2/spec_desc_plots/Modulation_descriptors', string(set(s)), '/',  class{c})
    if(isempty(dir(specFolder)))
        mkdir(specFolder);
    end
    name1 = strcat(string(set(s)), '_', string(i*2));
    extentionnew='.jpg';
